...
# -*- coding: utf-8 -*-
"""
Train CI prognosis model via causal hypergraph neural network.
"""
...
from src.models.losses import ConcordLossV2, ranking_distill_loss
...
SAVE_DIR = ROOT / cfg['paths']['results'] / 'result_CI'
CHECKPOINT_DIR = ROOT / cfg['paths']['checkpoints'] / 'CI'
...
def train_single_fold(args):
    ...
    nets = nn.ModuleList([HazardHGNN(i, has_cls=False).to(device_local) for i in range(4)])
    ...
    save_name = f'fold{fold_idx}_A{ALPHA:.1f}_B{BETA:.1f}_CI{best_ci:.4f}.pth'
    ...
    return {'fold': fold_idx, 'alpha': ALPHA, 'beta': BETA, 'ci': best_ci, 'save_path': str(save_path)}


def process_single_run():
    print('=' * 70)
    print(f'{CANCER} CI Prognosis Training')
    print('=' * 70)
    ...
    print(f'[{CANCER} result_CI] n_total: {n_total}, events: {int(e.sum())}')
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(outer_kf.split(range(n_total))):
        ...
        for inner_tr, _ in inner_kf.split(tr_idx):
            ...
            nets_tmp = nn.ModuleList([HazardHGNN(has_cls=False).to(device) for _ in range(4)])
    ...
    combo_results = defaultdict(lambda: {'cis': [], 'folds': [], 'paths': []})
    ...
    print(f'\n[{CANCER} result_CI] Top 10 Hyperparameter Combinations (based on 5-fold mean CI):')
    ...
    print(f'\n[{CANCER} result_CI] Best hyperparameters: alpha={best_A}, beta={best_B}')
    print(f'5-fold CI: {np.round(best_combo["fold_cis"], 4)}')
    print(f'Mean +/- Std: {best_combo["mean_ci"]:.4f} +/- {best_combo["std_ci"]:.4f}')

    print(f'\n[{CANCER} result_CI] Saving best models...')
    model_dir = SAVE_DIR / 'model_hgnn'
    ...
    final_summary = {
        ...
        'fold_cis': [float(c) for c in best_combo['fold_cis']],
    ...
    print(f'\n[{CANCER} result_CI] Results saved to: {SAVE_DIR}/')
