...
# -*- coding: utf-8 -*-
"""
Train CI prognosis model.
Grid search over alpha and beta with 5-fold CV.
Outputs: best_model_fold*.pth, final_summary.json under model_hgnn_v2/
"""
...
from src.models.losses import ranking_distill_loss_v2, ConcordLossV2
...
def train_single_fold(args_tuple):
    ...
    nets = nn.ModuleList([HazardHGNN(i, use_cls=False, drop_p=hgnn_cfg['drop_p']).to(device) for i in range(4)])
    ...
    best_ci = 0.0; counter = 0; best_state = None
    ...
    for epoch in range(1, hgnn_cfg['epochs'] + 1):
        ...
        loss_cox = criterion(haz_f, t_tr, e_tr, haz4_gated)
    ...
    return {'fold': fold_idx, 'alpha': alpha, 'beta': beta, 'ci': best_ci, 'best_state': best_state}


def process_single_run(cfg, cancer):
    print(f"\n[INFO] {cancer} | CI Training")
    ...
    for i, item in enumerate(combo_summary[:10]):
        print(f"  {i+1}. (alpha={item['alpha']}, beta={item['beta']}) -> Mean CI: {item['mean_ci']:.4f} ± {item['std_ci']:.4f} | Folds: {np.round(item['fold_cis'], 4).tolist()}")
    
    best_combo = combo_summary[0]
    best_A, best_B = best_combo['alpha'], best_combo['beta']
    print(f"\n[INFO] Best: alpha={best_A}, beta={best_B}")
    print(f"[INFO] Mean CI: {best_combo['mean_ci']:.4f} ± {best_combo['std_ci']:.4f}")
