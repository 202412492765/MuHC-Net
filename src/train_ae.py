...
torch.save({
    ...
    # Shared Decoders
    'dec_shared_mir': dec_shared_mir.state_dict(),
    'dec_shared_meth': dec_shared_meth.state_dict(),
    'dec_shared_mrna': dec_shared_mrna.state_dict(),
    
    # Private Decoders
    'dec_priv_mir': dec_priv_mir.state_dict(),
    'dec_priv_meth': dec_priv_meth.state_dict(),
    'dec_priv_mrna': dec_priv_mrna.state_dict(),
    
    # Cross-modal Predictors
    'pred_mir2meth': pred_mir2meth.state_dict(),
    'pred_mir2mrna': pred_mir2mrna.state_dict(),
    'pred_meth2mir': pred_meth2mir.state_dict(),
    'pred_meth2mrna': pred_meth2mrna.state_dict(),
    'pred_mrna2mir': pred_mrna2mir.state_dict(),
    'pred_mrna2meth': pred_mrna2meth.state_dict(),
    ...
    # Training metadata
    'epoch': max_epoch,
    'final_loss': float(log['loss'][-1]) if log['loss'] else None,
    'final_recon': float(log['recon'][-1]) if log['recon'] else None,
    'final_align': float(log['align'][-1]) if log['align'] else None,
    'final_alpha': {
        'mir': float(log['a_mir'][-1]) if log['a_mir'] else None,
        'meth': float(log['a_meth'][-1]) if log['a_meth'] else None,
        'mrna': float(log['a_mrna'][-1]) if log['a_mrna'] else None,
    }
