...
# Save directory for CI models
SAVE_DIR = OUTPUT_DIR / 'CI_model'
...
def delta_ci(haz4, t, e):
    """Calculate incremental importance for each modality"""
...
def train_single_fold(args):
    """
    Train specific fold with specific hyperparameters.
    Deep copy fix applied to ensure best model weights are preserved.
    """
    ...
    criterion = ConcordLossV2(alpha=ALPHA)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        haz4 = torch.cat([nets[i](inputs_tr[i], H_tr[i])[0] for i in range(4)], dim=1)
        haz4 = gate(haz4)
        haz_f, attn = fuse(haz4)
        
        loss_cox = criterion(haz_f, t_tr, e_tr, haz4)
        loss_dis = ranking_distill_loss(haz_f, haz4, t_tr, e_tr)
        
        logits = torch.stack([nets[i](inputs_tr[i], H_tr[i])[1] for i in range(4)], dim=1)
        logits = (attn.unsqueeze(-1) * logits).sum(dim=1)
        loss_cls = F.cross_entropy(logits, e_tr)
        ...
        loss = loss_cox + BETA * loss_dis + GAMMA * loss_cls + L1_LAMBDA * l1_reg
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                haz4_v = torch.cat([nets[i](inputs_val[i], H_val[i])[0] for i in range(4)], dim=1)
                haz4_v = gate(haz4_v)
                ...
                if ci > best_ci:
                    best_ci = ci
                    counter = 0
                    # Deep copy fix: detach and clone tensors to preserve exact best epoch weights
                    best_state = {
    ...
    # Save model only if save_path is provided (Phase 2)
...
def main():
    ...
    print(f'Hyperparameter search: 729 combinations x 5 folds = 3645 trainings')
    print('='*60)

    # Pre-calculate all fold data
    print('Pre-computing 5-fold data splits...')
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(outer_kf.split(range(n_total))):
        ...
        # Calculate delta_star using inner 5-fold
        ...
        for inner_tr, _ in inner_kf.split(tr_idx):
            ...
            for i in range(4):
                ...
                sub_h.append(torch.as_tensor(build_H(X_tmp), device=device))
        ...
        print(f'    Fold {fold_idx} delta_star: {delta_star}')
    ...
    # Phase 1: Generate all tasks for evaluation (no saving)
    ...
    # Parallel execution for evaluation
    ...
    with ctx.Pool(processes=MAX_WORKERS) as pool:
        for result in tqdm(
            ...
            total=len(all_tasks),
            desc='Global hyperparameter search'
        ):
            all_results.append(result)
    
    # Aggregate results by hyperparameter combo
    print('\nAggregating results, computing cross-fold mean CI...')
    combo_results = defaultdict(lambda: {'cis': [], 'folds': []})
    
    for res in all_results:
        key = (res['alpha'], res['beta'], res['gamma'])
        combo_results[key]['cis'].append(res['ci'])
        combo_results[key]['folds'].append(res['fold'])
    
    # Calculate mean CI and sort
    combo_summary = []
    for (A, B, G), data in combo_results.items():
        mean_ci = np.mean(data['cis'])
        std_ci = np.std(data['cis'])
    ...
    combo_summary.sort(key=lambda x: x['mean_ci'], reverse=True)
    
    # Output Top 10
    print('\nTop 10 hyperparameter combinations (based on 5-fold mean CI):')
    for i, item in enumerate(combo_summary[:10]):
        print(f'{i+1}. (A={item["alpha"]}, B={item["beta"]}, G={item["gamma"]}) '
              f'-> Mean CI: {item["mean_ci"]:.4f} ± {item["std_ci"]:.4f}')
    ...
    print(f'5-fold CI: {np.round(best_combo["fold_cis"], 4)}')
    print(f'Mean ± Std: {best_combo["mean_ci"]:.4f} ± {best_combo["std_ci"]:.4f}')
    
    # Phase 2: Retrain and save best models only (5 models total)
    print('\nSaving best models with deep copy fix...')
    for fold_idx in range(n_fold):
        fold_data = all_folds_data[fold_idx]
        save_path = SAVE_DIR / f'best_model_fold{fold_idx}.pth'
        
        # Retrain with best hyperparameters and save
        task = (fold_idx, best_A, best_B, best_G, fold_data, save_path)
        result = train_single_fold(task)
        
        print(f'  Fold {fold_idx}: {save_path} (CI={result["ci"]:.4f})')
    
    # Save summary
    final_summary = {
        'best_params': {'alpha': best_A, 'beta': best_B, 'gamma': best_G},
        'best_mean_ci': float(best_combo['mean_ci']),
        'best_std_ci': float(best_combo['std_ci']),
        'fold_cis': [float(x) for x in best_combo['fold_cis']],
    ...
    print('All models saved with deep copy fix ensuring reproducibility.')
    print('='*60)
